...
public class RetrievePDFManagementAdmin extends AppCompatActivity {
    ...
    private void deleteSelectedFile(String fileName) {
        ...
        managementReference.orderByChild("name").equalTo(fileName).addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
                ...
                if (dataSnapshot.exists()) {
                    for (DataSnapshot snapshot : dataSnapshot.getChildren()) {
                        ...
                        storageReference.delete().addOnSuccessListener(new OnSuccessListener<Void>() {
                            ...
                            public void onSuccess(Void aVoid) {
                                snapshot.getRef().removeValue()
                                        .addOnSuccessListener(new OnSuccessListener<Void>() {
                                            @Override
                                            public void onSuccess(Void aVoid) {
                                                Toast.makeText(RetrievePDFManagementAdmin.this, "Файл успішно видалено", Toast.LENGTH_SHORT).show();
                                            }
                                        })
                                        .addOnFailureListener(new OnFailureListener() {
                                            @Override
                                            public void onFailure(@NonNull Exception e) {
                                                Toast.makeText(RetrievePDFManagementAdmin.this, "Невдалось видалити файл: " + e.getMessage(), Toast.LENGTH_SHORT).show();
                                            }
                                        });
                            }
                        }).addOnFailureListener(new OnFailureListener() {
                            @Override
                            public void onFailure(@NonNull Exception e) {
                                Toast.makeText(RetrievePDFManagementAdmin.this, "Невдалось видалити файл із Firebase Storage: " + e.getMessage(), Toast.LENGTH_SHORT).show();
                            }
                        });
                    }
                } else {
                    Toast.makeText(RetrievePDFManagementAdmin.this, "Файл не знайдено", Toast.LENGTH_SHORT).show();
                }
            }

            @Override
            public void onCancelled(@NonNull DatabaseError databaseError) {
                Toast.makeText(RetrievePDFManagementAdmin.this, "Помилка: " + databaseError.getMessage(), Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    private void retrievePDFFiles() {
        ...
        englishReference.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                ArrayAdapter<putPDF> adapter = new ArrayAdapter<>(RetrievePDFManagementAdmin.this, android.R.layout.simple_list_item_1, pdfList);
