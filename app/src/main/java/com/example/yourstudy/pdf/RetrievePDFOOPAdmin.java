...
public class RetrievePDFOOPAdmin extends AppCompatActivity {
    ...
    private void deleteSelectedFile(String fileName) {
        ...
        OOPReference.orderByChild("name").equalTo(fileName).addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
                ...
                if (dataSnapshot.exists()) {
                    for (DataSnapshot snapshot : dataSnapshot.getChildren()) {
                        ...
                        storageReference.delete().addOnSuccessListener(new OnSuccessListener<Void>() {
                            ...
                            public void onSuccess(Void aVoid) {
                                snapshot.getRef().removeValue()
                                        .addOnSuccessListener(new OnSuccessListener<Void>() {
                                            @Override
                                            public void onSuccess(Void aVoid) {
                                                Toast.makeText(RetrievePDFOOPAdmin.this, "Файл успішно видалено", Toast.LENGTH_SHORT).show();
                                            }
                                        })
                                        .addOnFailureListener(new OnFailureListener() {
                                            @Override
                                            public void onFailure(@NonNull Exception e) {
                                                Toast.makeText(RetrievePDFOOPAdmin.this, "Невдалось видалити файл: " + e.getMessage(), Toast.LENGTH_SHORT).show();
                                            }
                                        });
                            }
                        }).addOnFailureListener(new OnFailureListener() {
                            @Override
                            public void onFailure(@NonNull Exception e) {
                                Toast.makeText(RetrievePDFOOPAdmin.this, "Невдалось видалити файл із Firebase Storage: " + e.getMessage(), Toast.LENGTH_SHORT).show();
                            }
                        });
                    }
                } else {
                    Toast.makeText(RetrievePDFOOPAdmin.this, "Файл не знайдено", Toast.LENGTH_SHORT).show();
                }
            }

            @Override
            public void onCancelled(@NonNull DatabaseError databaseError) {
                Toast.makeText(RetrievePDFOOPAdmin.this, "Помилка: " + databaseError.getMessage(), Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    private void retrievePDFFiles() {
        ...
        OOPReference.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                ArrayAdapter<putPDF> adapter = new ArrayAdapter<>(RetrievePDFOOPAdmin.this, android.R.layout.simple_list_item_1, pdfList);
