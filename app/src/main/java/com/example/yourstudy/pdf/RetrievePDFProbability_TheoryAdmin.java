...
public class RetrievePDFProbability_TheoryAdmin extends AppCompatActivity {
    ...
    private void deleteSelectedFile(String fileName) {
        ...
        probabilityReference.orderByChild("name").equalTo(fileName).addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
                ...
                if (dataSnapshot.exists()) {
                    for (DataSnapshot snapshot : dataSnapshot.getChildren()) {
                        ...
                        storageReference.delete().addOnSuccessListener(new OnSuccessListener<Void>() {
                            ...
                            public void onSuccess(Void aVoid) {
                                snapshot.getRef().removeValue()
                                        .addOnSuccessListener(new OnSuccessListener<Void>() {
                                            @Override
                                            public void onSuccess(Void aVoid) {
                                                Toast.makeText(RetrievePDFProbability_TheoryAdmin.this, "Файл успішно видалено", Toast.LENGTH_SHORT).show();
                                            }
                                        })
                                        .addOnFailureListener(new OnFailureListener() {
                                            @Override
                                            public void onFailure(@NonNull Exception e) {
                                                Toast.makeText(RetrievePDFProbability_TheoryAdmin.this, "Невдалось видалити файл: " + e.getMessage(), Toast.LENGTH_SHORT).show();
                                            }
                                        });
                            }
                        }).addOnFailureListener(new OnFailureListener() {
                            @Override
                            public void onFailure(@NonNull Exception e) {
                                Toast.makeText(RetrievePDFProbability_TheoryAdmin.this, "Невдалось видалити файл із Firebase Storage: " + e.getMessage(), Toast.LENGTH_SHORT).show();
                            }
                        });
                    }
                } else {
                    Toast.makeText(RetrievePDFProbability_TheoryAdmin.this, "Файл не знайдено", Toast.LENGTH_SHORT).show();
                }
            }

            @Override
            public void onCancelled(@NonNull DatabaseError databaseError) {
                Toast.makeText(RetrievePDFProbability_TheoryAdmin.this, "Помилка: " + databaseError.getMessage(), Toast.LENGTH_SHORT).show();
            }
        });
    }

    private void retrievePDFFiles() {
        DatabaseReference englishReference = databaseReference.child("Теорія ймовірностей");
        ...
        englishReference.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                ArrayAdapter<putPDF> adapter = new ArrayAdapter<>(RetrievePDFProbability_TheoryAdmin.this, android.R.layout.simple_list_item_1, pdfList);
